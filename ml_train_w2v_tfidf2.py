...
train_dwm = tfidf.fit_transform(train_X)
train_X_new = np.matmul(train_dwm.toarray(), word2vec_matrix)
train_X_new.shape

test_dwm = tfidf.fit_transform(test_X)
...
filename = './model/ml_model/LR_clf_w2v_tfidf_new.sav'
pickle.dump(clf, open(filename, 'wb'))

LR_scores = cross_val_score(clf, train_X_new, train_y, cv=10)
...
two_acc_small = return_two_calss_acc(clf, test_X_new, test_y)
print("Logistics Regression 2small class 정확도: {:.3f}".format(two_acc_small))

clf.fit(train_X_new, train_y_small)
filename = './model/ml_model/LR_clf_small_w2v_tfidf_new.sav'
pickle.dump(clf, open(filename, 'wb'))
...
two_acc_small = return_two_calss_acc(clf, test_X_new, test_y_small)
print("Logistics Regression 2small class 정확도: {:.3f}".format(two_acc_small))
